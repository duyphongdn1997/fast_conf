...
from src.data_processing.utils import split_into_segments, flatten, get_sentence_map, parse_args, BaseDocumentState
...
def get_document(instance, tokenizer, segment_len):
	...
	sentence_word_map = {}
	for sentence_idx, sentence in enumerate(instance["data"]):
	...
	dump_label, sent_idx, start_, end_ = [], 0, 0, 0
	for spans_label in instance["label"]:
		dump_label_span = []
		for (start, end) in spans_label:

			if end == -1:
				for sent in instance["data"]:
					end += len(sent)

			current_len = 0
			for idx, sent in enumerate(instance["data"]):
				# Check if the span in the sentence.
				if start >= current_len and end < current_len + len(sent):
					sent_idx = idx
					start_ = start - current_len
					end_ = end - current_len
					break  # Break to check the next span
				else:
					current_len += len(sent)

			dump_label_span.append([sent_idx, start_, end_])
		dump_label.append(dump_label_span)

	# Map preco clusters
	mapped_clusters = []
	for cluster in dump_label:
		cur_cluster = []
		for (sent_idx, word_start, word_end) in cluster:
			span_start = sentence_word_map[sent_idx][word_start][0]
			span_end = sentence_word_map[sent_idx][word_end][1]
